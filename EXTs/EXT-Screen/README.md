...
* **Minimal MagicMirror² version requirement: v2.30.0+bugsounet**
...
git clone https://github.com/bugsounet/MMM-Bugsounet/EXTs/EXT-Screen
cd EXT-Screen
npm run setup
```

`EXT-Screen` is now available for Windows environment!
...
* You can install it by using this command in your `EXT-Screen` folder: `npm run pinctrl`
...
⚠ **Pir Sensor is not available under windows environement**
 ...
 | Option  | Description | Type | Default |
 | ------- | --- | --- | --- |
 | animate | Animate EXT-Screen module on detect | Boolean | true |
...
* Available mode:
  ...
  * `mode: 1` - Wake up / turn OFF automaticaly by CRON and use countdown
    * `EXT-Screen` timer will be used before turn off screen
  ...
  * `mode: 2` - Your screen will be fully managed by cron
    * `EXT-Screen` timer will be not displayed and not used
...
* This module broadcasts:
  ...
  * `Bugsounet_SCREEN-POWER` with payload `true` when your screen turn on.
  * `Bugsounet_SCREEN-POWER` with payload `false` when your screen turn off.
  * `Bugsounet_USER-PRESENCE` with payload `true` when motion is detected.

* This module receive:

  * `Bugsounet_SCREEN-END` notification to force the end of the count down.
  * `Bugsounet_SCREEN-WAKEUP` notification to wake up the screen and reset count down.
  * `Bugsounet_SCREEN-LOCK` notification keep the screen on and lock it (freeze counter and stop pir detection).
  * `Bugsounet_SCREEN-UNLOCK` notification unlock the screen and restart counter and pir detection.
...
```sh
cd ~/MagicMirror/modules/MMM-Bugsounet/modules/MMM-Bugsounet/EXTs/EXT-Screen
npm run update
```
