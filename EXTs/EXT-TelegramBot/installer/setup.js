...
async function checkOS () {
  ...
  switch (sysinfo.type) {
    case "Linux":
      ...
      utils.empty();
      utils.info("➤ Install Emojis...");
      await utils.execCMD("mkdir ~/.fonts &>/dev/null");
      await utils.execCMD(`cp -f ${utils.getModuleRoot()}/installer/*.ttf ~/.fonts/`);
      await utils.execCMD("fc-cache -f -v &>/dev/null");
      utils.empty();
      utils.success("Done");
