...
class GOVERNOR {
  constructor (config, callback) {
    ...
    if (this.config.debug === true) log = (...args) => { console.log("[MMM-Screen] [LIB] [GOVERNOR]", ...args); };
    if (Utils.isWin()) {
      if (this.config.sleeping || this.config.working) {
        this.config.sleeping = 0;
        this.config.working = 0;
        console.log("[MMM-Screen] [LIB] [GOVERNOR] [Windows] Governor library Disabled");
      }
    }
    ...
    if (Utils.isWin()) {
      ...
      console.log("[MMM-Screen] [LIB] [GOVERNOR] Governor library initialized...");
    }
  }
  ...
  apply (type) {
    exec("cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor", (error, stdout) => {
      if (error) {
        ...
        console.error(`[MMM-Screen] [LIB] [GOVERNOR] ${type} - Error: ${this.Governor.error}`);
      ...
      if (this.Governor.actual === this.Governor.wanted) {
        ...
        if (!this.Governor.actived) {
          ...
          this.Governor.actived = false;
          console.error(`[MMM-Screen] [LIB] [GOVERNOR] ${type} ${this.Governor.error}`);
          this.error(this.Governor.error);
        }
      }
    });
  ...
  checkGovernor (wanted, type) {
    ...
    if (found) {
      ...
      console.error(`[MMM-Screen] [LIB] [GOVERNOR] ${type} Governor Error ! [${wanted}]`);
