...
class PIR {
  constructor (config, callback) {
    ...
    if (this.config.debug) log = (...args) => { console.log("[MMM-Screen] [LIB] [PIR]", ...args); };
    this.pir = null;
    this.running = false;
    this.pirChip = null;
    this.pirLine = null;
    ...
    if (Utils.isWin()) {
      console.log("[MMM-Screen] [LIB] [PIR] [Windows] Pir library Disabled.");
  ...
  start () {
    ...
    if (this.config.gpio === 0) return console.log("[MMM-Screen] [LIB] [PIR] Disabled.");
    switch (this.config.triggerMode) {
      case "LH":
        console.log("[MMM-Screen] [LIB] [PIR] triggerMode LH Selected: Read LOW (0, no-motion) to HIGH (1, motion)");
        break;
      case "H":
        console.log("[MMM-Screen] [LIB] [PIR] triggerMode H Selected: Read HIGH (1, motion)");
        break;
      default:
        console.warn(`[MMM-Screen] [LIB] [PIR] triggerMode: ${this.config.mode} is not a valid value`);
        console.warn("[MMM-Screen] [LIB] [PIR] set triggerMode LH");
        this.config.triggerMode = "LH";
        break;
    }
    switch (this.config.mode) {
      case 0:
        console.log("[MMM-Screen] [LIB] [PIR] Mode 0 Selected (gpiod library)");
        this.gpiodDetect();
        break;
      case 1:
        console.log("[MMM-Screen] [LIB] [PIR] Mode 1 Selected (gpiozero)");
        this.gpiozeroDetect();
        break;
      default:
        console.warn(`[MMM-Screen] [LIB] [PIR] mode: ${this.config.mode} is not a valid value`);
        console.warn("[MMM-Screen] [LIB] [PIR] set mode 0");
        this.config.mode = 0;
        this.gpiodDetect();
        break;
    }
  ...
  gpiozeroDetect () {
    ...
    console.log("[MMM-Screen] [LIB] [PIR] Started!");
    ...
    this.pir.on("message", (message) => {
      ...
      switch (message) {
        ...
        case "NoMotion":
          ...
          break;
        default:
          console.error("[MMM-Screen] [LIB] [PIR] ", message);
    ...
    this.pir.on("stderr", (stderr) => {
      ...
      if (this.config.debug) console.error("[MMM-Screen] [LIB] [PIR]", stderr);
      this.running = false;
    });

    this.pir.end((err, code, signal) => {
      if (err) {
        console.error("[MMM-Screen] [LIB] [PIR] [PYTHON]", err);
        this.callback("PIR_ERROR", err.message);
      }
      console.warn(`[MMM-Screen] [LIB] [PIR] [PYTHON] The exit code was: ${code}`);
      console.warn(`[MMM-Screen] [LIB] [PIR] [PYTHON] The exit signal was: ${signal}`);
    });
  }
    ...
    try {
      ...
      numbers.every((number) => {
        try {
          ...
          if (isAuto || isManual) {
            // found chip
            console.log(`[MMM-Screen] [LIB] [PIR] [GPIOD] - ${isAuto ? "Auto" : "Manual"} - Found chip ${number}: ${label}`);
      ...
      if (this.pirChipNumber === -1) {
        console.error("[MMM-Screen] [LIB] [PIR] [GPIOD] No Chip Found!");
        this.running = false;
      ...
      console.error(`[MMM-Screen] [LIB] [PIR] [GPIOD] ${err}`);
      this.running = false;
