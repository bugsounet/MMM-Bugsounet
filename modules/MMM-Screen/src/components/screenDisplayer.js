...
class screenDisplayer {
  constructor (config, Tools) {
    ...
    this.checkColor();
    console.log("[MMM-Screen] screenDisplayer Ready");
  }
  ...
  prepareDom () {
    ...
    if (this.style === "Text" && this.config.counter) {
      ...
      screen.id = "MMM-Screen_TEXT";
      var screenText = document.createElement("div");
      screenText.id = "MMM-Screen_TEXT_TRANSLATE";
      ...
      screenCounter.id = "MMM-Screen_TEXT_COUNTER";
      ...
      if (this.style !== "None") {
        ...
        screenStyle.id = "MMM-Screen_STYLE";
    ...
    if (this.config.lastPresence) {
      ...
      presence.id = "MMM-Screen_PRESENCE";
      presence.className = "hidden";
      var presenceText = document.createElement("div");
      presenceText.id = "MMM-Screen_PRESENCE_TEXT";
      ...
      presenceDate.id = "MMM-Screen_PRESENCE_DATE";
    ...
    if (this.config.availability) {
      ...
      availability.id = "MMM-Screen_AVAILABILITY";
      availability.classList.add("bright");
      var availabilityText = document.createElement("div");
      availabilityText.id = "MMM-Screen_AVAILABILITY_TEXT";
      ...
      availabilityValue.id = "MMM-Screen_AVAILABILITY_DATA";
  ...
  prepareStyle () {
    ...
    this.bar = new ProgressBar[this.style](document.getElementById("MMM-Screen_STYLE"), {
  ...
  updateDisplay (payload) {
    if (this.config.availability && payload.availability) {
      let availability = document.getElementById("MMM-Screen_AVAILABILITY_DATA");
    ...
    if (this.style === "Text") {
      if (this.config.counter) {
        let counter = document.getElementById("MMM-Screen_TEXT_COUNTER");
  ...
  updatePresence (payload) {
    ...
    if (this.userPresence) {
      let presence = document.getElementById("MMM-Screen_PRESENCE");
      presence.classList.remove("hidden");
      presence.classList.add("bright");
      let userPresence = document.getElementById("MMM-Screen_PRESENCE_DATE");
  ...
  screenShowing () {
    ...
    MM.getModules().enumerate((module) => {
      let speed = 200;
      let options = { lockString: "MMM-Screen_LOCK" };
  ...
  hideMe () {
    ...
    this.hide(1000, () => {}, { lockString: "MMM-Screen_LOCK", animate: "flipOutX" });
  }

  showMe () {
    if (!this.hidden()) return _logPIR("Already Showing.");
    this.show(1000, () => {}, { lockString: "MMM-Screen_LOCK", animate: "flipInX" });
  }
  ...
  checkStyle () {
    ...
    if (found) {
      this.style = found;
      console.log(`[MMM-Screen] Display.style: ${this.style}`);
    } else {
      console.error(`[MMM-Screen] Display.style Error ! [${this.config.style}]`);
      this.style = "Text";
    }
  }
  ...
  animateModule () {
    let animation = document.getElementById("MMM-Screen");
