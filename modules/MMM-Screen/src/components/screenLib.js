...
class SCREEN {
  constructor (config, callback) {
    ...
    if (this.config.debug) log = (...args) => { console.log("[MMM-Screen] [LIB] [SCREEN]", ...args); };
    this.screen = {
    ...
    switch (this.config.mode) {
      case 0:
        console.log("[MMM-Screen] [LIB] [SCREEN] Mode 0: Disabled");
        break;
      case 1:
        console.log("[MMM-Screen] [LIB] [SCREEN] Mode 1: dpms rpi");
        break;
      case 2:
        if (this.xrandrRotation.indexOf(this.config.xrandrForceRotation) === -1) {
          console.error(`[MMM-Screen] [LIB] [SCREEN] Mode 2: xrandr invalid Rotation --> ${this.config.xrandrForceRotation}, Set to default: normal`);
          ...
          console.log(`[MMM-Screen] [LIB] [SCREEN] Mode 2: xrandr (primary display) -- Rotation: ${this.config.xrandrForceRotation}`);
      ...
      case 3:
        if (this.wrandrRotation.indexOf(this.config.wrandrForceRotation) === -1) {
          console.error(`[MMM-Screen] [LIB] [SCREEN] Mode 3: wlr-randr invalid Rotation --> ${this.config.wrandrForceRotation}, Set to default: normal`);
          ...
          console.log(`[MMM-Screen] [LIB] [SCREEN] Mode 3: wlr-randr (primary display) -- Rotation: ${this.config.wrandrForceRotation}`);
          this.screen.wrandrRotation = this.config.wrandrForceRotation;
        }
        if (this.config.wrandrForceMode) {
          console.log(`[MMM-Screen] [LIB] [SCREEN] Mode 3: wlr-randr -- ForceMode: ${this.config.wrandrForceMode}`);
          this.screen.wrandrForceMode = this.config.wrandrForceMode;
        }
        if (this.config.waylandDisplayName.startsWith("wayland")) {
          console.log(`[MMM-Screen] [LIB] [SCREEN] Mode 3: wlr-randr -- DisplayName : ${this.config.waylandDisplayName}`);
          this.screen.waylandDisplayName = this.config.waylandDisplayName;
        } else {
          console.error(`[MMM-Screen] [LIB] [SCREEN] Mode 3: wlr-randr invalid DisplayName --> ${this.config.waylandDisplayName}, Set to default: wayland-0`);
          this.screen.waylandDisplayName = "wayland-0";
        }
        break;
      case 4:
        console.log("[MMM-Screen] [LIB] [SCREEN] Mode 4: HDMI CEC");
        break;
      case 5:
        console.log("[MMM-Screen] [LIB] [SCREEN] Mode 5: ddcutil");
        break;
      case 6:
        console.log("[MMM-Screen] [LIB] [SCREEN] Mode 6: dpms linux");
        break;
      case 7:
        if (this.config.waylandDisplayName.startsWith("wayland")) {
          console.log(`[MMM-Screen] [LIB] [SCREEN] Mode 7: labwc -- DisplayName : ${this.config.waylandDisplayName}`);
          this.screen.waylandDisplayName = this.config.waylandDisplayName;
        } else {
          console.error(`[MMM-Screen] [LIB] [SCREEN] Mode 7: labwc invalid DisplayName --> ${this.config.waylandDisplayName}, Set to default: wayland-0`);
          this.screen.waylandDisplayName = "wayland-0";
        }
        break;
      case 8:
        console.log("[MMM-Screen] [LIB] [SCREEN] Mode 8: relais");
        break;
      case 9:
        if (Utils.isWin()) {
          console.log("[MMM-Screen] [LIB] [SCREEN] Mode 9: Windows (test)");
        } else {
          console.error("[MMM-Screen] [LIB] [SCREEN] Mode 9 is reserved for windows OS -- Set mode to 0 (Disabled)");
          this.sendSocketNotification("SCREEN_ERROR", "Mode 9 is reserved for windows OS -- Set mode to 0 (Disabled)");
          this.config.mode = 0;
        }
        break;
      default:
        console.error(`[MMM-Screen] [LIB] [SCREEN] Unknow Mode (${this.config.mode}) Set to 0 (Disabled)`);
      ...
      if (this.config.mode) {
        if (this.config.mode === 5 && this.config.ddcutil.setPowerRetries > 0) {
          ...
          while (retries < this.config.ddcutil.setPowerRetries && !success) {
            console.info("[MMM-Screen] [LIB] [SCREEN] Retrying set power");
      ...
      switch (this.config.mode) {
        ...
        case 1:
          ...
          exec("DISPLAY=:0 xset q | grep Monitor", async (err, stdout) => {
            if (err) {
              console.error(`[MMM-Screen] [LIB] [SCREEN] ${err}`);
        ...
        case 2:
          ...
          exec("xrandr | grep 'connected primary'",
            async (err, stdout) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] xrandr: ${err}`);
        ...
        case 3:
          ...
          exec(`WAYLAND_DISPLAY=${this.screen.waylandDisplayName} wlr-randr | grep 'Enabled'`,
            (err, stdout) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] wlr-randr: ${err}`);
        ...
        case 4:
          ...
          exec("echo 'pow 0' | cec-client -s -d 1", async (err, stdout) => {
            if (err) {
              console.error(`[MMM-Screen] [LIB] [SCREEN] ${err}`);
              console.error(`[MMM-Screen] [LIB] [SCREEN] HDMI CEC Error: ${stdout}`);
        ...
        case 5:
          ...
          exec("ddcutil getvcp d6", async (err, stdout) => {
            if (err) {
              console.error(`[MMM-Screen] [LIB] [SCREEN] ddcutil Error ${err}`);
        ...
        case 6:
          ...
          exec("xset q | grep Monitor", async (err, stdout) => {
            if (err) {
              console.error(`[MMM-Screen] [LIB] [SCREEN] [Display Error] dpms linux: ${err}`);
        ...
        case 7:
          ...
          exec(`WAYLAND_DISPLAY=${this.screen.waylandDisplayName} wlopm --json`,
            async (err, stdout) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] wlopm: ${err}`);
                ...
                try {
                  ...
                  if (responseOutput.error) {
                    console.error(`[MMM-Screen] [LIB] [SCREEN] wlopm report arror: ${responseOutput.error}`);
                  ...
                  console.error(`[MMM-Screen] [LIB] [SCREEN] wlopm: ${error}`);
                  console.error(`[MMM-Screen] [LIB] [SCREEN] wlopm response: ${responseSh}`);
        ...
        case 8:
          ...
          exec(`pinctrl lev ${this.config.relayGPIOPin}`, async (err, stdout) => {
            if (err) {
              console.error(`[MMM-Screen] [LIB] [SCREEN] pinctrl get: ${err}`);
          ...
          break;
        case 9:
          console.log("[MMM-Screen] [LIB] [SCREEN] [Win Test] by pass check");
      ...
      switch (this.config.mode) {
        case 1:
          if (set) {
            exec("DISPLAY=:0 xset dpms force on", (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 1, power ON: ${err}`);
            ...
            exec("DISPLAY=:0 xset dpms force off", (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 1, power OFF: ${err}`);
        ...
        case 2:
          if (set) {
            exec(`xrandr --output ${this.screen.hdmiPort} --auto --rotate ${this.screen.xrandrRotation}`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 2, power ON: ${err}`);
            ...
            exec(`xrandr --output ${this.screen.hdmiPort} --off`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 2, power OFF: ${err}`);
        ...
        case 3:
          if (set) {
            ...
            exec(`WAYLAND_DISPLAY=${this.screen.waylandDisplayName} wlr-randr ${wrandrOptions}`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 3, power ON: ${err}`);
            ...
            exec(`WAYLAND_DISPLAY=${this.screen.waylandDisplayName} wlr-randr --output ${this.screen.hdmiPort} --off`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 3, power OFF: ${err}`);
        ...
        case 4:
          if (set) {
            exec("echo 'on 0' | cec-client -s", (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 4, power ON: ${err}`);
            ...
            exec("echo 'standby 0' | cec-client -s", (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 4, power OFF: ${err}`);
        ...
        case 5:
          if (set) {
            exec(`ddcutil setvcp d6 ${this.config.ddcutil.powerOnCode} --noverify`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 5, power ON: ${err}`);
                  ...
                  exec("ddcutil getvcp d6", (err, stdout) => {
                    if (err) {
                      ...
                      if (displaySh !== `0x${this.config.ddcutil.powerOnCode})`) {
                        console.error(`[MMM-Screen] [LIB] [SCREEN] ddcutil Error ${responseSh}`);
            ...
            exec(`ddcutil setvcp d6 ${this.config.ddcutil.powerOffCode} --noverify`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 5, power OFF: ${err}`);
        ...
        case 7:
          if (set) {
            exec(`WAYLAND_DISPLAY=${this.screen.waylandDisplayName} wlopm --on ${this.screen.hdmiPort}`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 7, power ON: ${err}`);
            ...
            exec(`WAYLAND_DISPLAY=${this.screen.waylandDisplayName} wlopm --off ${this.screen.hdmiPort}`, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 7, power OFF: ${err}`);
        ...
        case 8:
          if (set) {
            ...
            exec(cmd, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 8, power ON: ${err}`);
            ...
            exec(cmd, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 8, power OFF: ${err}`);
        ...
        case 9:
          if (set) {
            ...
            exec(ps, { shell: "powershell.exe" }, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 9, power ON: ${err}`);
            ...
            exec(ps, { shell: "powershell.exe" }, (err) => {
              if (err) {
                console.error(`[MMM-Screen] [LIB] [SCREEN] mode 9, power OFF: ${err}`);
